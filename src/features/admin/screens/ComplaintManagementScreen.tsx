...
export default function ComplaintManagementScreen() {
  ...
  const renderFilterPills = () => (
    <View style={s.filterRow}>
      ...
      >
        <Text
          style={[TextStyles.label, { color: filterStatus === "ALL" ? "#fff" : colors.textSecondary }]}
        >
          All
        </Text>
      </Pressable>
  ...
  const renderItem = ({ item }: { item: Complaint }) => {
    ...
    return (
      <Card variant="elevated" style={[s.card, { borderLeftWidth: 3, borderLeftColor: STATUS_COLORS[item.status] }]}>
        ...
        <View style={s.cardHeader}>
          <View style={s.titleRow}>
            ...
            <Badge
              ...
              variant={item.status === "RESOLVED" ? "success" : item.status === "REJECTED" ? "error" : "brand"}
              size="sm"
            />
          </View>
          <View style={s.metaRow}>
            ...
            <Text style={[TextStyles.caption, { color: colors.textTertiary }]}>· {item.category}</Text>
          </View>
        </View>
        ...
        {item.aiPrediction && (
          <View style={[s.aiStrip, { backgroundColor: colors.brand + "10" }]}>
            ...
            <Text style={[TextStyles.caption, { color: colors.brand }]}>
              YOLO11 · {item.aiPrediction.damageClass} ·{" "}
              {(aiConf! * 100).toFixed(1)}% confidence
            </Text>
          ...
          <View style={s.footerRight}>
            <View style={[s.slaBadge, { backgroundColor: slaColor + "15" }]}>
              ...
              <Text style={[TextStyles.caption, { color: slaColor }]}>
                SLA {slaStatus === "breached" ? "⚠ Breached" : slaStatus === "warning" ? "At Risk" : "OK"}
              </Text>
            </View>
            <Pressable
              style={({ pressed }) => [
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.bgBase }]}>
      ...
      {/* Search */}
      <View style={[s.searchRow, { backgroundColor: colors.bgSurface, borderColor: colors.borderDefault }]}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={s.emptyState}>
              ...
              <Text style={[TextStyles.subtitle, { color: colors.textSecondary, marginTop: Spacing[3] }]}>
                No complaints found
              </Text>
              <Text style={[TextStyles.caption, { color: colors.textTertiary }]}>
