    ...
    if (inferenceUrl) {
      try {
        ...
        const response = await firstValueFrom(
          this.httpService.post(`${inferenceUrl}/analyze`, dto, { headers, timeout: 5000 })
        );

        const data = response.data;
        if (data && data.department) {
          this.logger.log(`Received successful response from FastAPI inference service.`);
          return {
            success: true,
            triageResult: {
              ...
              aiSummary: data.aiSummary || `AI analyzed the text and image and categorized it as ${data.category}.`,
              detections: data.detections || [],
            }
          };
        }
      } catch (error) {
        this.logger.error(`Failed to call FastAPI inference service: ${error.message}. Falling back to mock model.`);
      }
    }
