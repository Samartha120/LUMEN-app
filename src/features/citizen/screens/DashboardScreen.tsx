...
const AI_INSIGHTS = [
  { icon: "trend", text: "Water complaints increased 17% this week. Suggest inspection on Hosur Rd.", action: "View Hotspot" },
  { icon: "activity", text: "3 high-priority issues near you are unresolved for over 48 hours.", action: "View Issues" },
  { icon: "spark", text: "Pothole repairs on MG Road are 70% complete. Estimated finish: 3 hrs.", action: "Track Progress" },
];
...
function AnimatedCounter({ target, duration = 1200, suffix = "" }: { target: number; duration?: number; suffix?: string }) {
  ...
  useEffect(() => {
    Animated.timing(anim, { toValue: target, duration, useNativeDriver: false, easing: Easing.out(Easing.cubic) }).start();
...
export default function CitizenDashboardScreen() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      Animated.spring(statsScale, { toValue: 1, useNativeDriver: true, speed: 12, bounciness: 8 }).start();
    }, 200);

    setTimeout(() => {
      Animated.spring(actionsSlide, { toValue: 0, useNativeDriver: true, speed: 14, bounciness: 6 }).start();
    }, 350);
  ...
  // ── Sub-Components ──
  const MetricCell = ({
    label, value, icon, color, trend, suffix = "",
  }: { label: string; value: number; icon: any; color: string; trend: string; suffix?: string }) => (
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.bgBase }]}>
      ...
      <LinearGradient
        colors={isDark
          ? ["#0B1827", colors.bgBase]
          : ["#E8F4FF", "#F0F7FF", colors.bgBase]}
        start={{ x: 0, y: 0 }}
        end={{ x: 0.3, y: 0.5 }}
      ...
      <View
        pointerEvents="none"
        style={[s.orb, { backgroundColor: colors.brand + "18" }]}
      />
      ...
      >
        ...
        <Animated.View style={[s.header, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
          <View style={s.headerLeft}>
            <View style={s.greetingRow}>
              ...
              <Text style={[TextStyles.caption, { color: colors.textTertiary, fontWeight: "600", letterSpacing: 1.5 }]}>
                LIVE DASHBOARD
              </Text>
            </View>
          ...
          <View style={s.headerRight}>
            <Pressable
              style={({ pressed }) => [
                s.iconBtn,
                { backgroundColor: colors.bgSurface, borderColor: colors.borderDefault, opacity: pressed ? 0.7 : 1, transform: [{ scale: pressed ? 0.94 : 1 }] },
              ]}
        ...
        {/* ALERT BANNER                                    */}
        {/* ═══════════════════════════════════════════════ */}
        <Animated.View style={[s.section, { transform: [{ translateY: alertSlide }], opacity: fadeIn }]}>
          <LinearGradient
          ...
          >
            ...
            <View style={s.alertLeft}>
              <Animated.View style={[s.alertDot, { backgroundColor: "#FCA5A5", transform: [{ scale: pulsAnim }] }]} />
        ...
        <Animated.View style={[s.section, { opacity: fadeIn, transform: [{ scale: statsScale }] }]}>
          ...
          <BlurView intensity={isDark ? 30 : 15} tint={isDark ? "dark" : "light"} style={[s.glassContainer, { borderColor: colors.borderDefault }]}>
            <LinearGradient
              colors={isDark ? ["rgba(255,255,255,0.04)", "rgba(255,255,255,0.01)"] : ["rgba(255,255,255,0.85)", "rgba(255,255,255,0.6)"]}
              style={StyleSheet.absoluteFill}
            />
            <View style={s.metricsGrid}>
              <MetricCell label="Reports Filed" value={12} icon="reportList" color="#208AEF" trend="+3 this week" />
              <View style={[s.metricDivider, { backgroundColor: colors.borderDefault }]} />
              <MetricCell label="Resolved" value={7} icon="success" color="#12B76A" trend="+2 this week" />
              <View style={[s.metricHDivider, { backgroundColor: colors.borderDefault }]} />
              <MetricCell label="Pending" value={3} icon="timer" color="#F79009" trend="−1 vs last week" />
              <View style={[s.metricDivider, { backgroundColor: colors.borderDefault }]} />
              <MetricCell label="Civic Score" value={87} icon="star" color="#7C3AED" trend="+5 pts" suffix="%" />
            </View>
          </BlurView>
        </Animated.View>
        ...
        {/* ═══════════════════════════════════════════════ */}
        <Animated.View style={[s.section, { transform: [{ translateY: actionsSlide }], opacity: fadeIn }]}>
          <Text style={[s.sectionTitle, { color: colors.textPrimary, marginBottom: 16 }]}>Quick Actions</Text>
          <View style={s.actionsGrid}>
            {QUICK_ACTIONS.map((action, idx) => (
              <ActionCard key={action.label} action={action} colors={colors} isDark={isDark} idx={idx} />
            ))}
          </View>
        </Animated.View>
        ...
        <Animated.View style={[s.section, { opacity: fadeIn }]}>
          <BlurView intensity={isDark ? 40 : 20} tint={isDark ? "dark" : "light"} style={[s.aiCard, { borderColor: colors.brand + "30" }]}>
            <LinearGradient
              colors={isDark ? ["rgba(32,138,239,0.08)", "rgba(124,58,237,0.05)"] : ["rgba(235,245,255,0.95)", "rgba(245,243,255,0.85)"]}
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
              style={StyleSheet.absoluteFill}
            />
            ...
            <View style={s.aiInner}>
              <View style={s.aiHeader}>
                ...
                <View style={s.aiTitleBlock}>
                  ...
                  <Text style={[TextStyles.caption, { color: colors.textTertiary }]}>Updating every 5s</Text>
                </View>
                <Animated.View style={[s.aiPulse, { backgroundColor: "#12B76A", transform: [{ scale: pulsAnim }] }]} />
              </View>

              <Animated.Text style={[s.aiInsightText, { color: colors.textPrimary, opacity: aiOpacity }]}>
                {insight.text}
              </Animated.Text>

              <View style={s.aiActions}>
                <Pressable style={[s.aiActionBtn, { backgroundColor: colors.brand + "15", borderColor: colors.brand + "30" }]}>
                  <Text style={[TextStyles.caption, { color: colors.brand, fontWeight: "700" }]}>{insight.action}</Text>
                  <LumenIcon name="arrowLeft" size="xs" color={colors.brand} />
                </Pressable>
                <Pressable style={[s.aiActionBtn, { backgroundColor: colors.bgSubtle, borderColor: colors.borderDefault }]}>
                  <Text style={[TextStyles.caption, { color: colors.textSecondary, fontWeight: "600" }]}>Dismiss</Text>
                </Pressable>
              </View>
            </View>
          </BlurView>
        ...
        <Animated.View style={[s.section, { opacity: fadeIn }]}>
          <View style={s.sectionHeader}>
            ...
            <Pressable style={[s.viewAllBtn, { borderColor: colors.borderDefault }]}>
              <Text style={[TextStyles.caption, { color: colors.brand, fontWeight: "700" }]}>View Map</Text>
              <LumenIcon name="arrowLeft" size="xs" color={colors.brand} />
            </Pressable>
          </View>

          <Pressable onPress={() => router.push("/(citizen)/Report-details" as any)}>
            <BlurView intensity={isDark ? 25 : 10} tint={isDark ? "dark" : "light"} style={[s.mapCard, { borderColor: colors.borderDefault }]}>
              <LinearGradient
              ...
              <View pointerEvents="none" style={StyleSheet.absoluteFill}>
                {[0.25, 0.5, 0.75].map((f, i) => (
                  <View key={i} style={[s.gridLine, s.gridH, { top: `${f * 100}%` as any, borderColor: colors.borderDefault + "30" }]} />
                ))}
                {[0.25, 0.5, 0.75].map((f, i) => (
                  <View key={i} style={[s.gridLine, s.gridV, { left: `${f * 100}%` as any, borderColor: colors.borderDefault + "30" }]} />
                ))}
              </View>
              {/* Issue markers */}
              {[
              ...
              >
                <View style={s.mapOverlayContent}>
                  <View style={s.mapStatRow}>
                    <View style={[s.mapStat, { backgroundColor: "#F04438" + "20", borderColor: "#F04438" + "40" }]}>
                      <Text style={[s.mapStatNum, { color: "#F04438" }]}>5</Text>
                      <Text style={[s.mapStatLabel, { color: "#F04438" }]}>High</Text>
                    </View>
                    <View style={[s.mapStat, { backgroundColor: "#F79009" + "20", borderColor: "#F79009" + "40" }]}>
                      <Text style={[s.mapStatNum, { color: "#F79009" }]}>8</Text>
                      <Text style={[s.mapStatLabel, { color: "#F79009" }]}>Medium</Text>
                    </View>
                    <View style={[s.mapStat, { backgroundColor: "#12B76A" + "20", borderColor: "#12B76A" + "40" }]}>
        ...
        <Animated.View style={[s.section, { opacity: fadeIn }]}>
          <View style={s.sectionHeader}>
            ...
            >
              <Text style={[TextStyles.caption, { color: colors.brand, fontWeight: "700" }]}>View all</Text>
              <LumenIcon name="arrowLeft" size="xs" color={colors.brand} />
            </Pressable>
          </View>
      ...
      <View style={s.fabContainer}>
        ...
        >
          ...
          <View style={[s.fabLabel, { backgroundColor: colors.bgSurface, borderColor: colors.borderDefault }]}>
            <Text style={[TextStyles.caption, { color: colors.textPrimary, fontWeight: "700" }]}>Report Issue</Text>
          </View>
        </Pressable>
      </View>
    </View>
...
function ActionCard({ action, colors, isDark, idx }: { action: any; colors: any; isDark: boolean; idx: number }) {
    ...
    <Animated.View style={{ transform: [{ scale }], width: ACTION_W }}>
      ...
      >
        ...
        <View style={ac.textBlock}>
          ...
          <Text style={[ac.subtitle, { color: colors.textTertiary }]} numberOfLines={1}>{action.subtitle}</Text>
        </View>
...
const s = StyleSheet.create({
  ...
  iconBtn: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center", borderWidth: 1, position: "relative" },
  notifBadge: { position: "absolute", top: 8, right: 8, width: 16, height: 16, borderRadius: 8, alignItems: "center", justifyContent: "center" },
  ...
  viewAllBtn: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 10, paddingVertical: 5, borderWidth: 1, borderRadius: 20 },

  // Alert Banner
  alertBanner: { borderRadius: 18, padding: 16, flexDirection: "row", alignItems: "center", gap: 12, overflow: "hidden" },
  alertLeft: { flexDirection: "row", alignItems: "center", gap: 8 },
  alertDot: { width: 8, height: 8, borderRadius: 4 },
  alertBody: { flex: 1 },
  alertTitle: { fontSize: 11, fontWeight: "800", color: "#FCA5A5", letterSpacing: 1.2, marginBottom: 2 },
  ...
  alertEtaLabel: { fontSize: 9, color: "rgba(255,255,255,0.7)", fontWeight: "600", letterSpacing: 0.5 },
  ...
  aiActionBtn: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 12, borderWidth: 1 },

  // Map Card
  mapCard: { height: 200, borderRadius: 24, overflow: "hidden", borderWidth: 1, position: "relative" },
  ...
  markerDot: { width: 28, height: 28, borderRadius: 14, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: "rgba(255,255,255,0.4)" },
  ...
  barsContainer: { flexDirection: "row", alignItems: "flex-end", gap: 8, height: 140, paddingTop: 8 },
  barWrap: { flex: 1, alignItems: "center", gap: 6 },
  bar: { width: "100%" },
  barLabel: { fontSize: 10, fontWeight: "600" },
  analyticsStats: { flexDirection: "row", justifyContent: "space-around", paddingTop: 4, borderTopWidth: 1, borderTopColor: "rgba(0,0,0,0.06)" },
  ...
  // FAB
  fabContainer: { position: "absolute", bottom: 24, right: 24, flexDirection: "row", alignItems: "center", gap: 10 },
  fab: { shadowColor: "#F04438", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 },
...
const ac = StyleSheet.create({
  ...
  arrow: { width: 24, height: 24, borderRadius: 12, alignItems: "center", justifyContent: "center", alignSelf: "flex-end" },
});
